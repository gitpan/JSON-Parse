...
and the missing second half of a surrogate pair:
    ...
    validate_json ('["\udc00? <-- should be a second half here"]');

gives output

    JSON error at line 1, byte 9/44: Unexpected character '?' parsing string starting from byte 5: expecting '\' 
...
the string, it will give an error like this:
    ...
    validate_json ('{"first":"Suzuki","second":"Murakami","third":"Asada}');
...
a C file F<Json3-random-test.c> which isn't in the CPAN distribution,
...
A pure C version called "random-test.c" also exists. This applies
