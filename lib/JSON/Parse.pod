...
the UTF-8 encoding, and is opened as a character file using the
":encoding(utf8)" pragma. The output is marked as character strings.
...
Surrogate pairs in the form C<\uD834\uDD1E> are also handled.
...
which caused the problem. The line number is formed simply by counting
the number of "\n" (linefeed, ASCII 0x0A) characters in the whitespace
part of the JSON. If you find the error message unclear, please report
that as a bug.
...
Alternatively, the likely syntax of the various error messages is also
visible in the file F<t/valid-json.t>.
...
More extensive testing code is in the github repository which is not
supplied with the module itself. A script, F<randomjson.pl>, generates
a set number of bytes of random JSON and checks that the module's
bytewise validation of input is correct. For example, one gigabyte of
random bytes can be validated. This relies on a C file
F<Json3-random-test.c> and it also requires F<Json3.xs> to be edited
to make the macro C<TESTRANDOM> true (uncomment line 7 of the
file). The testing code deliberately creates erroneous input and then
uses C setjmp/longjmp to replace Perl croak, so it's not guaranteed to
