...
C<parse_json> throws a fatal error ("dies").
...
to the equivalent ASCII character. If the input string is marked as
Unicode, Unicode escape characters of the form \uXXXX (see page three
of L</RFC 4627>) are mapped to UTF-8. If the input string is not
marked as Unicode, they are left untouched.
...
module than with L<JSON::XS>. The parsing of exponentials is done by
the system's C<strtod> function in in JSON::Parse, but JSON::XS
contains its own exponential parser.
